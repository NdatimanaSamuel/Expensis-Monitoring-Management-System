                  ...
                  <div class="form-group">
                    ...
                    <div class="form-row">
                      ...
                      <label for="projectcost">Requested By:</label>
                         <input class="form-control input-sm" id="projectcost" name="request_by"  type="text" required step=".01" min="0">
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="form-row">
                      ...
                      <label for="projectcost">Requested On:</label>
                         <input class="form-control input-sm" id="projectcost" name="request_on"  type="date" required step=".01" min="0">
                      </div>
                    </div>
                  </div>
...
if ($StatusProject=="FINISHED") {
 ...
 exit();
  ...
  if (!preg_match("/^[a-zA-Z\s]+$/",$request_by)) {
  echo"<script>alert('Requested By Name Must Be Character');</script>"; 
 exit();
 }
    ...
    $remainingmoney=$project_cost-$totalExpenseMoney;

                                 

$conn->query("UPDATE `project` SET   `project_expense`='$totalExpenseMoney' ,`remaindmoney`='$remainingmoney'WHERE `project_id` = '$ID'") or die(mysqli_error());

$insert=mysqli_query($conn,"INSERT INTO expense(project_id,expense_type,expense_item,seller,date_given,expense, requesteby,describe_expense,done_by)
  VALUES('$project_id','$expense_type','$item_name','$seller_name','$request_on','$expense_cost','$request_by','$describe_expense','$doneBY')");
...
echo " <script>window.alert('successful Expenses Added On Project');window.location='viewProject.php'</script>";
}

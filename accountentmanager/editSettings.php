...
if (strlen($PhoneNumber)>14) {
    ...
    exit();
        ...
        $conn->query("UPDATE `account` SET   `fullname`='$FullName',`email` = '$Email', `phone` = '$PhoneNumber',`password`='$Password' ,`address`= '$Address' WHERE `user_id` = '$ID'") or die(mysqli_error());

        echo '<script>alert("Profile Updated");window.location=\'settings.php\';</script>';
